...
if [ "$k8s_dns" = "yes" ]; then
    echo "Setting the k8s dns server in nextepc"
    sed -i 's/PRIMARY_DNS_REPLACE/'$(cat /etc/resolv.conf | grep nameserver | awk '{print $NF}')'/g' /etc/nextepc/nextepc.conf
else
    echo "Setting the default dns server in nextepc"
    sed -i 's/PRIMARY_DNS_REPLACE/'"8.8.8.8"'/g' /etc/nextepc/nextepc.conf
fi


# it seems there is no support of ipv6 in containers, to solve (if needed) https://ask.openstack.org/en/question/68190/how-do-i-resolve-rtnetlink-permission-denied-error-encountered-while-running-stacksh/
